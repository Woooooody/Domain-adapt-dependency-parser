...
def orthonormal_VanillaLSTMBuilder(lstm_layers, input_dims, lstm_hiddens, pc):
    """
    标准正交
    :param lstm_layers:Number of layers
    :param input_dims:Dimension of the input
    :param lstm_hiddens:Dimension of the recurrent units
    :param pc:parameters collection
    :return:
    """
    ...
    for layer, params in enumerate(builder.get_parameters()):
        ...
        The output is a list with one item per layer.
        Each item is a list containing Wx,Wh,b
        where Wx,Wh are stacked version of the individual gates matrices:
              h/x
            +------+
            |      |
        i   |      |
        ...
        params[0].set_value(np.concatenate([W_x] * 4, 0))  # Wx : stacked version of the individual gates matrix
        params[1].set_value(np.concatenate([W_h] * 4, 0))  # Wh : stacked version of the individual gates matrix
        b = np.zeros(4 * lstm_hiddens, dtype=np.float32)
        b[lstm_hiddens:2 * lstm_hiddens] = -1.0
        params[2].set_value(b)  # b
    return builder
...
def biLSTM(builders, inputs, batch_size=None, dropout_x=0., dropout_h=0.):
    ...
    return inputs, fs, bs  # 142*800*18
