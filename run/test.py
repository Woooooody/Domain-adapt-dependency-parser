...
def test(parser, vocab, num_buckets_test, test_batch_size, test_file, output_file):
    ...
    for words, tags, arcs, rels in data_loader.get_batches(batch_size=test_batch_size, shuffle=False):
        dy.renew_cg()
        common_top_recur, private_top_recur,_,_ = parser.run_lstm(words, tags, isTrain=False)
        outputs = parser.run_parser(words, common_top_recur, private_top_recur, isTrain=False)
...
if __name__ == '__main__':
    ...
    parser = Parser(vocab, config.word_dims, config.tag_dims, config.dropout_emb, config.lstm_layers, config.lstm_hiddens, config.dropout_lstm_input, config.dropout_lstm_hidden, config.mlp_arc_size, config.mlp_rel_size, config.dropout_mlp,config.domain_num)
