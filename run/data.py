...
class Vocab(object):
	PAD, ROOT, UNK, BOS, EOS = 0, 1, 2, 3, 4
	...
	def __init__(self, input_file, pret_file = None, min_occur_count = 2):
		...
		self._id2word = ['<pad>', '<root>', '<unk>','<bos>','<eos>']
	...
	def rel2id(self, xs):
		...
		if xs not in self._rel2id:
			print(xs)
			return self._rel2id['<pad>']
...
class DataLoader(object):
	def __init__(self, input_file, n_bkts, vocab):
		...
		self._record = []
		for sent in sents:
			bkt_idx = len2bkt[len(sent)] # 找到句子长度对应的桶号
			self._buckets[bkt_idx].append(sent) # 将对应那个长度的句子放入相应的桶中
			idx = len(self._buckets[bkt_idx]) - 1 # 桶中个数-1
